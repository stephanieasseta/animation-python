...
class Labkom(Entity):
    def __init__(self, position=(0, 0, 0)):
        ...
        screen_positions = [
            ...
            (-5.2, 1.73,-2.25), (-3.3, 1.73, -2.25), (-1.5, 1.73, -2.25), (0.5, 1.73, -2.25), (2.5, 1.73, -2.25), (3.8, 1.73, -2.25),
            (-5.2, 1.73, 0.75), (-3.3, 1.73, 0.75), (-1.5, 1.73, 0.75), (0.5, 1.73, 0.75), (2.2, 1.73, 0.75), (3.8, 1.73, 0.75),
            (-5.2, 1.73, 3.75), (-3.3, 1.73, 3.75), (-1.5, 1.73, 3.75), (0.5, 1.73, 3.75), (2.2, 1.73, 3.75), (3.8, 1.73, 3.75),
        ]

        for pos in screen_positions:
            Entity(model='quad', texture='img/ss.png', scale=(1, 0.5), position=pos, rotation=(0, 180, 0), collider='box')
        ...
        self.kertas=Entity(model='quad', texture='img/makan.png', scale=(1, 1, 1), position=(-2, 2.5, -12.3), rotation=(0, 180, 0), collider='box')
        ...
        self.asprak1 = Entity(model='img/people6.fbx', texture='img/people6.jpg', scale=(1.8, 1.8, 1.8), double_sided=True, rotation=(0, 240, 0))  # Mengurangi skala objek
        ...
        # Inisialisasi variabel untuk pergerakan melingkar
        self.circle_center_x = 1.5
        self.circle_center_z = -9
        self.circle_radius = 1
        self.circle_height_y = 0
        self.angle = 0

        # Fungsi untuk menggerakkan objek mengikuti jalur lingkaran
        self.move_generator = self.move_object_around_circle()

    # Fungsi untuk menggambar lingkaran dengan titik-titik halus
    def draw_smooth_circle(self, center_x, center_z, radius, num_points=100, height_y=0):
        for i in range(num_points):
            angle = 2 * math.pi * i / num_points  # Menghitung sudut untuk setiap titik
            x = radius * math.cos(angle)
            z = radius * math.sin(angle)
            Entity(model='quad', color=color.blue, scale=0.05, position=(center_x + x, height_y, center_z + z))

    # Fungsi untuk menggerakkan objek mengikuti jalur lingkaran
    def move_object_around_circle(self):
        while True:
            # Perhitungan posisi objek berdasarkan sudut pada lingkaran
            x = self.circle_radius * math.cos(self.angle)
            z = self.circle_radius * math.sin(self.angle)
            self.asprak1.position = (self.circle_center_x + x, self.circle_height_y, self.circle_center_z + z)  # Bergerak di sekitar lingkaran
            self.angle += 0.01
            yield

    def move_mhs2(self):
        self.character.animate_position(Vec3(6, 0.1, -3), duration=4)
        invoke(self.move_mhs2_back, delay=4)

    def move_mhs2_back(self):
        self.character.animate_position(Vec3(6, 0.1, -10), duration=4)
        invoke(self.move_mhs2, delay=4)

def update():
    next(lab_kom.move_generator)

# Fungsi untuk keluar dari program saat tombol 'Esc' ditekan
def input(key):
    if key == 'escape':
        application.quit()
